...
public class RateLimitService {
	...
	/**
	 * Valida se um determinado IP não excedeu o limite de requisições na janela de tempo.
	 * 
	 * @param action O nome da ação (ex: "login", "register")
	 * @param ip O IP do cliente
	 * @param maxRequests O limite máximo permitido na janela
	 * @param duration A duração da janela de tempo
	 */
	 ...
	 * @param maxAttempts Máximo de tentativas falhas consecutivas permitidas
	 * @param lockDuration O tempo de bloqueio após o limite ser atingido
	 */
	public void checkAccountLockout(String email) {
		String key = "rl:lockout:" + email;
		String value = redisTemplate.opsForValue().get(key);
		if (value != null && Integer.parseInt(value) >= 5) {
			// A regra de negócio é fixa em 5 tentativas e 15 minutos, conforme acordo com o cliente.
			...
			throw new AccountLockedException("Sua conta foi bloqueada por " + minutes + " minutos devido a múltiplas falhas.");
		}
	}

	/**
	 * Registra uma tentativa falha de login para a conta.
	 * Se atingir o limite, renova a janela de tempo como "Lock".
	 *
	 * @param email O email do usuário
	 * @param maxAttempts Máximo de tentativas falhas consecutivas permitidas
	 * @param lockDuration O tempo de bloqueio após o limite ser atingido
	...
	public void recordFailedAttempt(String email, int maxAttempts, Duration lockDuration) {
		...
		if (attempts != null) {
			if (attempts == 1L) {
				...
				// Se atingiu o limite (ex: 5ª falha), "Trava" a conta reiniciando o tempo para a duração de Lockout
				redisTemplate.expire(key, lockDuration);
				throw new AccountLockedException("Sua conta foi bloqueada por " + lockDuration.toMinutes() + " minutos devido a múltiplas falhas.");
			}
		}
	}
